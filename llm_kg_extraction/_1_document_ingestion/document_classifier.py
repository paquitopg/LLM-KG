...
class DocumentClassifier:
    ...
    def classify_document(self,
        ...
        # Handle case where text extraction fails (e.g., image-based PDFs)
        if text_for_summary is None or text_for_summary.strip() == "":
            print("DocumentClassifier: No text content available, generating image-based summary...")
            document_summary = self._generate_image_based_summary(pdf_parser, num_pages_for_summary)
        else:
    ...
    def _generate_image_based_summary(self, pdf_parser, num_pages: int = 4) -> Optional[Dict[str, Any]]:
        """
        Generates a summary based on PDF page images when text extraction fails.
        
        Args:
            pdf_parser: The PDF parser instance
            num_pages (int): Number of first pages to use for summary generation
            
        Returns:
            Optional[Dict[str, Any]]: Dictionary with 'summary' and optional 'main_entity' fields,
                                    or None if generation fails.
        """
        print("DocumentClassifier: Generating image-based document summary...")
        
        try:
            # Extract images from the first N pages
            images_base64 = pdf_parser.extract_images_from_first_n_pages_base64(num_pages)
            
            if not images_base64:
                print("DocumentClassifier: No images could be extracted for summary generation.")
                return self._create_fallback_summary("No text or images available for summary generation.")
            
            # Check if the LLM client supports multimodal
            if hasattr(self.summary_llm_client, 'chat_completion') and hasattr(self.summary_llm_client, 'supports_multimodal'):
                if self.summary_llm_client.supports_multimodal():
                    # Create multimodal prompt with images
                    prompt = f"""
                    Please analyze these PDF page images and provide a concise summary of the key topics, purpose, and overall nature of the document.
                    The summary should be dense and present the main subjects or actors of the document.
                    IT WILL BE USED AS CONTEXT TO GUIDE KNOWLEDGE EXTRACTION, SO ONLY THE MAIN POINTS ARE NEEDED.

                    I also need to know the main entity or subject of the document, if applicable.
                    For instance, if the document is about a company, the main entity would be the company name.
                    If the document is a legal contract, the main entity could be the parties involved or the contract type.
                    ...
                    # Create multimodal input with images based on LLM client type
                    if isinstance(self.summary_llm_client, VertexLLM):
                        llm_input_parts = [{'text': prompt}]
                        ...
                        # Use generate_content for Vertex AI with parts
                        raw_response = self.summary_llm_client.generate_content(
                            prompt=llm_input_parts,
                            temperature=self.summary_temperature
                        )
                    elif isinstance(self.summary_llm_client, AzureLLM):
                        multimodal_content = [
                        ...
                        # Generic fallback for other LLM clients
                        messages = [{"role": "user", "content": prompt}]
                        raw_response = self.summary_llm_client.chat_completion(
                            messages=messages,
                            temperature=self.summary_temperature
                    ...
                    if not raw_response or not raw_response.strip():
                        print("DocumentClassifier: Generated image-based summary was empty or whitespace.")
                        return None
                    
                    # Clean and parse response (same logic as text-based summary)
                    cleaned_response = raw_response.strip()
                    ...
                    try:
                        ...
                        if not isinstance(summary_dict, dict) or "summary" not in summary_dict:
                            print("DocumentClassifier: Invalid image-based summary response structure.")
                            return self._create_fallback_summary(raw_response)
                        
                        if "main_entity" not in summary_dict:
                            summary_dict["main_entity"] = None
                        
                        print(f"DocumentClassifier: Image-based summary generated successfully: {summary_dict['summary']}...")
                        if summary_dict.get("main_entity"):
                            print(f"DocumentClassifier: Main entity identified: {summary_dict['main_entity']}")
                        ...
                        print(f"DocumentClassifier: Failed to parse image-based summary JSON: {json_error}")
                        return self._create_fallback_summary(raw_response)
                else:
                    print("DocumentClassifier: Summary LLM client does not support multimodal processing.")
                    return self._create_fallback_summary("Document has no extractable text and summary LLM does not support images.")
            else:
                print("DocumentClassifier: Summary LLM client does not support multimodal processing.")
                return self._create_fallback_summary("Document has no extractable text and summary LLM does not support images.")
                
        except Exception as e:
            print(f"Error generating image-based document summary: {e}")
            return self._create_fallback_summary(f"Failed to generate image-based summary: {e}")
