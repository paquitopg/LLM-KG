...
from _4_knowledge_graph_operations.common_kg_utils import normalize_entity_ids, clean_knowledge_graph, filter_irrelevant_entities
...
class KGConstructorSingleDoc:
    ...
    def _finalize_document_kg(self, extracted_kgs: List[Dict[str, Any]], 
        ...
        # Clean and normalize the final document KG
        final_document_kg = clean_knowledge_graph(final_document_kg)
        final_document_kg = normalize_entity_ids(final_document_kg)
        
        # Apply quality filtering to remove irrelevant entities
        main_entity_name = self.document_context_info.get("main_entity", "")
        if main_entity_name:
            final_document_kg = filter_irrelevant_entities(
                final_document_kg, 
                main_entity_name,
